...
// TODO: This solution can be fixed, the code is of poor quality
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        // u8 is between 0-255
        let a = u8::try_from(tuple.0).unwrap();
        let b = u8::try_from(tuple.1).unwrap();
        let c = u8::try_from(tuple.2).unwrap();
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let a = u8::try_from(arr[0]).unwrap();
        let b = u8::try_from(arr[1]).unwrap();
        let c = u8::try_from(arr[2]).unwrap();
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        let a = u8::try_from(slice[0]).unwrap();
        let b = u8::try_from(slice[1]).unwrap();
        let c = u8::try_from(slice[2]).unwrap();
